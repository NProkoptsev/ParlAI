    ...
    def __init__(self, opt, shared=None):
        ...
        if shared:
            ...
            if not self.attention:
                ...
                self.attn2 = nn.Linear(hsz, hsz)
                self.attn2.weight.data = torch.eye(hsz)
                self.attn_combine = nn.Linear(2* hsz + emb, emb)
...
class PersonachatSeqseqAgentSplit(Agent):
        ...
        if shared:
            ...
            if not self.attention:
                ...
                if self.opt['personachat_attnsentlevel']:
                    ...
                    self.attn2 = nn.Linear(emb, emb)
                    self.attn2.weight.data = torch.eye(emb)
                    self.attn_h2attn = nn.Linear(hsz, emb)
                    self.attn_combine = nn.Linear(emb + emb + emb, emb)
                else:
    ...
    def _apply_attention(self, xes, persona_output,encoder_output, hidden, attn_mask=None, attn_mask2=None):
        """Apply attention to encoder hidden layer."""
        if self.attention.startswith('concat'):
            hidden_expand = hidden[-1].unsqueeze(1).expand(hidden.size()[1], encoder_output.size()[1], hidden.size()[2])
            attn_w_premask = self.attn_v(F.tanh(self.attn(torch.cat((encoder_output, hidden_expand), 2)))).squeeze(2)
        ...
        if self.attention.startswith('general'):
            ...
            attn_w_premask2 = torch.bmm(self.attn2(hidden_expand), persona_output.transpose(1, 2)).squeeze(1)
            attn_w_premask2 = attn_w_premask * attn_mask2.float() - (1 - attn_mask2.float()) * 1e20
            attn_weights2 = F.softmax(attn_w_premask2)
        ...
        attn_applied = torch.bmm(attn_weights.unsqueeze(1), encoder_output).squeeze(1)
        attn_applied2 = torch.bmm(attn_weights2.unsqueeze(1), persona_output).squeeze(1)
        output = torch.cat((xes[0], attn_applied, attn_applied2), 1)
        output = self.attn_combine(output).unsqueeze(0)
        output = F.tanh(output)

        return output, attn_weights, attn_w_premask, attn_w_premask2

    def _decode_and_train(self, batchsize, xes, ys, encoder_output_persona,encoder_output, hidden_persona, hidden, attn_mask,attn_mask2, guide_indices):
        # update the model based on the labels
        self.zero_grad()
        loss = 0
        loss_guide = 0
        ...
        for i in range(ys.size(1)):
            ...
            if self.attention:
                ...
                output, attn_weights, attn_w_premask, attn_w_premask2 = self._apply_attention(xes, encoder_output_persona,encoder_output, h, attn_mask, attn_mask2)
            else:
                output = xes
    ...
    def _decode_perp(self, batchsize, xes, ys, encoder_output_persona, encoder_outpu, hidden_persona, hidden, attn_mask, attn_mask2, zs):
        # calculate the perplexity
        log_perp = 0.
        for i in range(zs.size(1)):
            if type(self.decoder) == nn.LSTM:
            ...
            if self.attention:
                if self.opt['personachat_attnsentlevel']:
                    h = self.attn_h2attn(h)
                output, attn_weights, attn_w_premask, attn_w_premask2 = self._apply_attention(xes, encoder_output_persona,encoder_outpu, h, attn_mask)
            else:
                output = xes
    ...
    def _decode_only(self, batchsize, xes, ys, encoder_output_persona, encoder_output, hidden_persona, hidden, attn_mask, attn_mask2, zs):
        ...
        while(total_done < batchsize) and max_len < self.longest_label:
            ...
            if self.attention:
                if self.opt['personachat_attnsentlevel']:
                    h = self.attn_h2attn(h)
                output, attn_weight, _,_ = self._apply_attention(xes, encoder_output_persona,encoder_output, h, attn_mask, attn_mask2)
            else:
                output = xes
    ...
    def _score_candidates(self, cands, xe, encoder_output_persona, encoder_output, hidden_persona, hidden, attn_mask, attn_mask2):
        # score each candidate separately
        ...
        for i in range(cview.size(1)):
            if self.attention:
                if self.opt['personachat_attnsentlevel']:
                    ...
                    h = cands_hn
                output, _ , _, _ = self._apply_attention(cands_xes, cands_encoder_output_persona,cands_encoder_output, h, cands_attn_mask, cands_attn_mask2)
            else:
                output = cands_xes
    ...
    def predict(self, xs, xs_persona, ys=None, cands=None, zs=None):
        ...
        attn_mask2 = Variable(xs.data.ne(0), requires_grad=False)
        if is_training:
            output_lines = self._decode_and_train(batchsize, xes, ys,
                                                  encoder_output_persona,encoder_output, hidden_persona, hidden, attn_mask,attn_mask2, guide_indices)

        else:
            if cands is not None:
                text_cand_inds = self._score_candidates(cands, xe,
                                                        encoder_output_persona, encoder_output, hidden_persona, hidden, attn_mask, attn_mask2)

            output_lines = self._decode_only(batchsize, xes, ys,
                                             encoder_output_persona, encoder_output, hidden_persona, hidden, attn_mask, attn_mask2, zs)
            if zs is not None:
                self._decode_perp(batchsize, xes, ys,
                                  encoder_output_persona, encoder_output, hidden_persona, hidden, attn_mask, attn_mask2, zs)
